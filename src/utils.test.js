...
test('sortArrayBuffers', t => {
  ...
  const order = sortArrayBuffers(hexToArrayBuffer(a), hexToArrayBuffer(b))
  t.is(order, -1)
})

test('mergeArrayBuffers', t => {
  const a = hexToArrayBuffer('4fe9367ef5dad459') // 5758193517488428121
  const b = hexToArrayBuffer('546266e73438ef2d') // 6080535590103215917
  const c = hexToArrayBuffer('4fe9367ef5dad459546266e73438ef2d')
  t.deepEqual(mergeArrayBuffers(a, b), c)
})

test('mergeAllArrayBuffers', t => {
  const a = hexToArrayBuffer('4fe9367ef5dad459')
  const b = mergeAllArrayBuffers(
    ...['4f', 'e9', '36', '7e', 'f5', 'da', 'd4', '59'].map(hexToArrayBuffer),
  )
  t.true(arrayBufferEqualTo(a, b))
})

test('arrayBufferEqualTo', t => {
  const a = hexToArrayBuffer('4fe9367ef5dad459')
  const b = hexToArrayBuffer('4fe9367ef5dad459')
  const c = hexToArrayBuffer('546266e73438ef2d')
  t.true(arrayBufferEqualTo(a, b))
  t.false(arrayBufferEqualTo(a, c))
})

test('hashLeaf', t => {
  const addr = '4fe9367ef5dad459ae9cc4265c69b1b10a4e1288'
  const a = utils.keccak256('0x' + addr)
  const b = '0x' + arrayBufferToHex(hashLeaf(hexToArrayBuffer(addr)))
  t.is(a, b)
})

test('hashParent', t => {
  const addr0 = '4fe9367ef5dad459ae9cc4265c69b1b10a4e1288'
  const addr1 = '546266e73438ef2486c6d004d4cb8995373512da'
  const a = utils.keccak256('0x' + addr0 + addr1)
  const b =
    '0x' +
    arrayBufferToHex(
      hashParent(hexToArrayBuffer(addr0), hexToArrayBuffer(addr1)),
    )
  const c =
    '0x' +
    arrayBufferToHex(
      hashParent(hexToArrayBuffer(addr1), hexToArrayBuffer(addr0)),
    )
  t.is(a, b)
  t.is(a, c)
})
...
test('verifyProof', t => {
  const addr0 = hexToArrayBuffer('4fe9367ef5dad459ae9cc4265c69b1b10a4e1288')
  const addr1 = hexToArrayBuffer('546266e73438ef2486c6d004d4cb8995373512da')
  const leaf0 = hashLeaf(addr0)
  const leaf1 = hashLeaf(addr1)
  const root = hashParent(leaf0, leaf1)
  const valid = verifyProof(addr0, leaf1, root)
  t.true(valid)
})
